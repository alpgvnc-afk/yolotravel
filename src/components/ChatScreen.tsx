...
export default function ChatScreen({
  ...
  return (
    <div className="flex h-screen flex-col bg-[#0a0a0a] text-neutral-100">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-white/5 px-6 py-4 pt-12">
      ...
      {hasRecommendations && (
        <button
          ...
          className="mx-6 mt-4 flex items-center justify-center gap-2 rounded-2xl bg-amber-400 py-3 text-sm font-bold text-black shadow-md glow-amber active:scale-[0.98] transition-transform"
        >
        ...
        <AnimatePresence initial={false}>
            ...
            >
              ...
              <div className="flex flex-col gap-1 max-w-[80%]">
                <div className={`rounded-2xl px-4 py-3 text-base whitespace-pre-line ${
                  msg.sender === 'ai'
                    ? 'bg-[#141414] border border-white/5 text-neutral-100 rounded-tl-none'
                    : 'bg-amber-400 text-black rounded-tr-none'
                }`}>
                  {msg.text}
                </div>
                <span className={`text-[10px] text-neutral-500 ${msg.sender === 'user' ? 'text-right' : ''}`}>
                  {msg.timestamp}
                </span>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {msg.options.map((opt) => (
                      <button
                        ...
                        className="rounded-xl border border-amber-400/60 bg-amber-400/10 px-4 py-2 text-sm font-semibold text-amber-300 hover:bg-amber-400/20 transition-colors"
                      >
                        {opt}
                      </button>
                    ))}
                    <button
                      ...
                      className="rounded-xl border border-white/10 bg-[#141414] px-4 py-2 text-sm font-semibold text-neutral-300 hover:bg-[#1c1c1c] transition-colors"
                    >
          ...
          {loading && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-3">
              ...
              <div className="rounded-2xl rounded-tl-none bg-[#141414] border border-white/5 px-4 py-3">
          ...
          {error && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="self-center rounded-xl bg-red-950/40 border border-red-500/40 px-4 py-2 text-sm text-red-300 max-w-[90%]">
              {error}
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {/* Input */}
      <div className="p-4 pb-24 border-t border-white/5 bg-[#0a0a0a]">
        <div className="flex items-center gap-3 rounded-full bg-[#141414] border border-white/10 px-4 py-1 focus-within:border-amber-400/60 transition-colors">
          <input
            ref={inputRef}
            type="text"
            value={inputValue}
            ...
            disabled={loading}
            className="flex-1 py-2 bg-transparent text-white outline-none placeholder:text-neutral-500 disabled:opacity-50"
          />
          <Mic className="h-5 w-5 text-neutral-500 cursor-pointer" />
          <button
