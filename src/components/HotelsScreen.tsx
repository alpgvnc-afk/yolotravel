      ...
      <div className="px-6 pt-12 pb-6">
        <div className="flex items-center gap-4 mb-2">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-black">{destination.name}</h1>
            <p className="text-sm text-gray-500">{t('hotelTitle')}</p>
          </div>
        </div>
        ...
        {error && (
          <div className="space-y-3 my-4">
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 rounded-2xl bg-amber-400 py-4 text-base font-bold text-black active:scale-[0.98] transition-transform"
            >
          ...
          <div className="grid gap-5 mt-4">
            {hotels.map((hotel, i) => (
              <motion.div
                ...
                className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-md shadow-black/5"
              >
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-xs text-gray-500 ml-1">{hotel.rating.toFixed(1)}</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-gray-400">{t('perNight')}</p>
                      <p className="text-lg font-bold text-black">{hotel.currencySymbol || '$'}{hotel.pricePerNight.toLocaleString()}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex gap-2">
                    <button onClick={() => onSelect(hotel)} className="flex-1 rounded-2xl border-2 border-amber-400 bg-white py-3 text-sm font-bold text-black active:scale-[0.98] transition-transform">
                      {t('detail')}
                    </button>
                    <a href={hotel.bookingUrl || '#'} target="_blank" rel="noopener noreferrer" className="flex-1 rounded-2xl bg-amber-400 py-3 text-sm font-bold text-black text-center active:scale-[0.98] transition-transform">
